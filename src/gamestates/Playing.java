...
public class Playing extends VBox {
    ...
    public Playing(Game game) {
        ...
        setBackground(new Background(new BackgroundImage(new Image(MENU_BACKGROUND, MENU_BACKGROUND_WIDTH, 0, true, false), null, null, null, null)));
    ...
    public void update(int levelNum) {
        ...
        // Reset previous level in case "NEXT LEVEL" button is pressed before completing the level
        // While calling this method from level menu, if Level 1 is called currentLevelNum-2 becomes -1.
        // So the method gives an IndexOutOfBound error. To prevent this we added an if condition.
        ...
        // If next level exists, check if it is unlocked and set color of NEXT LEVEL label to green or red
    ...
    // Drive button action
    public void drive() {
        // If animation is not running and destination of vehicle is not null, start animation
        if (!levels.get(currentLevelNum-1).getAnimation().isPlaying() && levels.get(currentLevelNum-1).getVehicle().getDestination()!= null) {
